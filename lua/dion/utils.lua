...
nvim.install = nvim_installation or { home = stdpath "config" }
--- external nvim configuration folder
...
function nvim.conditional_func(func, condition, ...)
...
function nvim.trim_or_nil(str) return type(str) == "string" and vim.trim(str) or nil end
...
function nvim.notify(msg, type, opts)
...
function nvim.confirm_prompt(messages)
  if messages then nvim.echo(messages) end
...
local function user_setting_table(module)
  ...
  local settings = nvim.user_settings or {}
...
function nvim.initialize_packer()
  -- try loading packer
  ...
  if packer_avail then
    ...
    local run_me, _ = loadfile(
      nvim.user_plugin_opts("plugins.packer", { compile_path = nvim.default_compile_path }).compile_path
    )
    ...
    -- if there is no compiled file, prompt the user to run :PackerSync
      ...
      nvim.echo { { "Please run " }, { ":PackerSync", "Title" } }
    end
  end
end
...
function nvim.vim_opts(options)
...
function nvim.url_opener()
  ...
  -- if any other operating system notify the user that there is currently no support
    ...
    nvim.notify("gx is not supported on this OS!", "error")
  end
end
...
function nvim.toggle_term_cmd(term_details)
  ...
  if nvim.user_terminals[term_key] == nil then
    nvim.user_terminals[term_key] = require("toggleterm.terminal").Terminal:new(term_details)
  end
  -- toggle the terminal
  nvim.user_terminals[term_key]:toggle()
end

--- Add a source to cmp
-- @param source the cmp source string or table to add (see cmp documentation for source table format)
function nvim.add_cmp_source(source)
...
function nvim.get_user_cmp_source(source)
  ...
  local priority = nvim.user_plugin_opts("cmp.source_priority", {
    nvim_lsp = 1000,
    luasnip = 750,
    buffer = 500,
    path = 250,
...
function nvim.add_user_cmp_source(source) nvim.add_cmp_source(nvim.get_user_cmp_source(source)) end
...
function nvim.which_key_register(mappings, opts)
...
function nvim.null_ls_providers(filetype)
  local registered = {}
  -- try to load null-ls
...
function nvim.null_ls_sources(filetype, method)
  local methods_avail, methods = pcall(require, "null-ls.methods")
  return methods_avail and nvim.null_ls_providers(filetype)[methods.internal[method]] or {}
end
...
function nvim.alpha_button(sc, txt)
...
function nvim.is_available(plugin) return packer_plugins ~= nil and packer_plugins[plugin] ~= nil end
...
function nvim.set_mappings(map_table, base)
...
function nvim.delete_url_match()
...
function nvim.set_url_match()
  nvim.delete_url_match()
  if vim.g.highlighturl_enabled then vim.fn.matchadd("HighlightURL", nvim.url_matcher, 15) end
end

--- Toggle URL/URI syntax highlighting rules
function nvim.toggle_url_match()
...
function nvim.cmd(cmd, show_error)
